...
class SDKAddonPreferences(AddonPreferences):
    ...
    def sdk_path_update(self, context):
        ...
        try:
            ...
            addon_dir_name = (
                os.path.basename(addon_path).replace("-", "_").replace(".", "_")
            )
    ...
    def draw(self, context):
        ...
        if self.sdk_path:
            ...
            if not sdk_exists:
                layout.label(
                    text="SDK path does not exist. Please install the SDK.",
                    icon="ERROR",
                )
            else:
                ...
                if has_python and has_runtime:
                    ...
                    layout.label(
                        text="SDK path exists but missing required folders (python/, runtime/)",
                        icon="ERROR",
                    )
                    layout.label(
                        text="This should point to the add-on directory, not the UPBGE directory."
                    )
