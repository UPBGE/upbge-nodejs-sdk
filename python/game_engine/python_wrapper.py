...
def _build_context_from_module():
    \"\"\"Wrapper that imports and uses the centralized context builder module.\"\"\"
    try:
        # Try to import from the SDK's context builder module
        try:
            from upbge_nodejs_sdk.python.game_engine.context_builder import build_context
        except ImportError:
            ...
            from python.game_engine.context_builder import build_context

        return build_context()
    except Exception:
        # If import fails, return empty context
        return {{
            "scene_name": "",
            "object_name": "",
            "position": None,
...
try:
    ...
    if SCRIPT_NAME in bpy.data.texts:
        ...
        if is_javascript_file(script_name):
            ...
            ctx = _build_context_from_module()
