...
def execute(context, is_interactive):
    ...
    if line.strip():
        ...
        if is_multiline:
            ...
            bpy.ops.console.history_append(
                text=indent, current_character=0, remove_duplicates=True
            )
    ...
    code_to_execute = (
        console_state["accumulated_code"] if console_state["accumulated_code"] else line
    )
    ...
    if not code_to_execute.strip():
        ...
        sc.prompt = PROMPT
        bpy.ops.console.history_append(
            text="", current_character=0, remove_duplicates=True
        )
        return {"FINISHED"}
    ...
    output, error_output, success = execute_javascript(
        code_to_execute, is_multiline=False, context_id=context_id
    )
    ...
    if node_path:
        try:
            ...
            result = subprocess.run(
                [node_path, "--version"], capture_output=True, text=True, timeout=2
            )
